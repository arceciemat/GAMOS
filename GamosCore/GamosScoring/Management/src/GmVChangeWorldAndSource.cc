...
GmVChangeWorldAndSource::GmVChangeWorldAndSource() : GmUserEventAction(), GmUserStackingAction()
{
  ...
  theNEventsPerRun = G4int(GmParameterMgr::GetInstance()->GetNumericValue("GmVChangeWorldAndSource:nEventsPerRun",100));
...
{
  if( bTooManyEvents ) {
    G4int nevtmax = theNEventsPerRun * theWorlds.size() * theSourceCommands.size();
