...
beginT = clock();
  ...
  if (span==0 && MRDiff==-1) MRDiff = n_planes - 1; 

  cout << "ssrb::input parameters: rings = " << n_planes << "; bins = " << n_bin << "; ang_views = " << n_ang << " .. pixDim: "<< pixDim << " ringDim: " << sliceDim << endl;
  ...
  for(i=0;i<n_bin;i++){
	...
	plane2D=(int)(k%n_planes + k/n_planes);  // SSRB: p2D = p3D%n_planes + p3D/n_planes --> max. 2*n_planes - 1
	//Max Ring Difference:
	if (abs((k/n_planes - k%n_planes)) <= MRDiff) {  // Z1= p3D / n_planes; Z2= p3D % n_planes
	  planesNorm[plane2D]++;
	  sino2D[i][j][plane2D]=(float)sino3D[i][j][k]+ sino2D[i][j][plane2D];
	  //  cout << "3Dp= " << k << " plane2D= "<< plane2D << endl;
	}
      }
      // }
      
      /* else{
	int offset= (int)((n_planes_file - 2*n_planes + 1)/2 );  
	int lastSegP;
	k=0;
	for(int s=0;s<=nSEG;s++){   //TO DO: añadir aqui MRDiff!!!!
	  if(s==0){
	    while(k<2*n_planes-1){
	    planesNorm[k]++;
	    sino2D[i][j][k]=(float)sino3D[i][j][k];
	    k++;
	    }
	  }else{
	  plane2D=s*span-1;
	    while(k<lastSegP + 2*(n_planes+1-span*s)-1){
	      planesNorm[plane2D]+=2;
	      sino2D[i][j][plane2D]= sino2D[i][j][plane2D] + (float)sino3D[i][j][k] + (float)sino3D[i][j][k+offset];
	      k++;
	      plane2D++;
	    } 
	  }
...
// --- data from SSRB to FBP part:
 ...
 //     delta_slice=sliceDim;
 delta_slice = sliceDim*0.5;        // Corrected 2020-04-30
...
// ------------------------------ end SSRB part ----------------------------------- //
  ...
  cout << "fbp2D::output      image: X and Y dimension = " << M << "; pixel size = " << delta_x << endl;
  cout << "... " << endl;
  cout << "fbp2D::filter parameters: Window type " << type_f << "; alpha = " << alpha << "; Butterworth order = " << Butt_n << endl;
  cout << "... " << endl;
  ...
  // Mem. allocation of in_data, out_data (temporal data fftw_complex) and plan (fftw_plan), to be used for filtering with FFTW libraries (type: fftw_complex)
  ...
  in_data = (double*) fftw_malloc(sizeof(double)*P);
  out_data = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*Pd2);
  
  plan_D = fftw_plan_dft_r2c_1d(P,in_data,out_data, FFTW_ESTIMATE);
  plan_I = fftw_plan_dft_c2r_1d(P,out_data,in_data, FFTW_ESTIMATE);

  //(*)FFTW NOTE: if input data in[i] are purely real numbers, the DFT output satisfies the "Hermitian" redundancy: out[i] is the conjugate of out[n-i].
  //    (for multidimensional: For example, consider the transform of a two-dimensional real array of size n0 by n1. 
  //    The output of the r2c transform is a two-dimensional complex array of size n0 by n1/2+1, where the y dimension has been cut nearly in half because of redundancies in the output.)
...
beginT_AfterINIT = clock();
  ...
  for(k=0;k<n_slices;k++){            // FOR every slice
                                      // !!! TO DO: compute in parallel CPUs
	...
	in_data[i] = (double)sino2D[i][j][k];   //real part
      ...
      for(i=n_bin;i<P;i++){                  //FOR zero padding, using R!=2^n  (and P=2^n)
	in_data[i] = 0.0;           	
      }
...
elapTime = ((double)(endT - beginT))/CLOCKS_PER_SEC;
double elapTime_perSlice = ( (double)(beginT_AfterINIT - beginT + endT - endT_AFTER_wr) + (double)(endT_AFTER_wr - beginT_AfterINIT)/n_slices )/CLOCKS_PER_SEC; 
...
void read_sino3D(){
  ...
  for(i=0;i<n_bin;i++){
    ...
    for(j=0;j<n_ang;j++){
      ...
      /// !!!! DECIDIR si almacenar MAX_Ring difference o dejar asi con el tamaño incluyendo todos los planos, y =0 en caso > MRDiff
      for(k=0;k<(n_planes_file);k++){
...
void write_INTERFILE(const char* filename, float*** data, int x, int y, int z, double pixD, double sliceD){   //TO DO: sacar x, y, z directament del array "data"
 ...
 float *data_line; 
 data_line = (float*) malloc( x*y*z*sizeof(float) );
...
void free_data() {       // liberar memoria
int i,j;
