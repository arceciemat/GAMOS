...
//-----------------------------------------------------------------------------
void Dicom2G4FileMgr::AddFileInterfile( G4String fileName )
{
  DicomReaderInterfile* df = new DicomReaderInterfile(fileName);
  if( fCompression != 1 ) df->SetCompression( fCompression );
...
void Dicom2G4FileMgr::ProcessFiles()
{
  ...
  std::vector<DicomVReaderImage*> IFReaders = theReaderMgr->GetImageReaders(DRM_Interfile, false);
  theIFReaderAll = 0;
  //  G4cout << " GDEB Dicom2G4FileMgr::ProcessFiles " << CTReaders.size() << G4endl; //GDEB
  if( IFReaders.size() != 0 ) {
      if( theCTReaderAll != 0 ){
