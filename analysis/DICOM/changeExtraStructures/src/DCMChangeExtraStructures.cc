#include "DCMChangeExtraStructures.hh"
...
//---------------------------------------------------------------------------
DCMChangeExtraStructures::DCMChangeExtraStructures()
{
  Initialise();
...
//---------------------------------------------------------------------------
void DCMChangeExtraStructures::Initialise()
{
...
//---------------------------------------------------------------------------
void DCMChangeExtraStructures::ProcessArguments(int argc,char** argv)
{
  if( argc == 6 ) {
	...
	bImageLimits = true;	  
	ii++;
      } else if( argvstr == "-fCT2HU" ) {
	theCT2HUFN = GmGenUtils::GetValue(argv[ii+1]);
	ii++;
      } else {
...
//---------------------------------------------------------------------------
void DCMChangeExtraStructures::CheckArguments()
{
  ...
  //--- THERE MUST BE AN RTSTRUCT FILE  
  size_t nfst = theParamMgr->GetNofImageFiles("fRTStruct");
  if( nfst != 1 ) {
    G4Exception(theExeName, 
		"",
		FatalException,
		("Number of input files of type DICOM RTSTRUCT is " + GmGenUtils::itoa(nfct) + ", while it should be 1").c_str());  // Only G4dcmCT containing material information
  }

  //--- THERE CANNOT BE ANY OTHER FILE
  size_t nother = theParamMgr->GetNofImageFiles("ANY")-nfct-nfst;
  if( nother != 0 ) { //- if there is one image file not G4dcmCT, it will be used as image to be changed
    G4Exception(theExeName,
		"",
		FatalException,
		("Number of input files of type different than DIM_G4dcmCT and DIM_RTStruct is " + GmGenUtils::itoa(nother) + ", while it should be 0").c_str()); 
  }
  ...
  if (theCT2HUFN == "" ) {
    G4Exception(theExeName,
		"",
		FatalException,
		"There must be a file for CT2HU conversion");
  }
...
//---------------------------------------------------------------------------
void DCMChangeExtraStructures::ReadFilesAndGetImages()
{
...
//---------------------------------------------------------------------------
void DCMChangeExtraStructures::BuildStructData()
{
...
//---------------------------------------------------------------------------
void DCMChangeExtraStructures::ChangeImages()
{
  if( !bImageLimits ) {
  ...
  ChangeVoxelsPartialStruct();

}

//---------------------------------------------------------------------------
void DCMChangeExtraStructures::ChangeImagesByStruct()
{
  //--- LOOP TO VOXELS TO CHANGE MATERIAL OR DENSITY
  G4int nVoxels = theAnalyseImage->GetNoVoxels();
  //  G4cout << " DCMChangeExtraStructures::ChangeImagesByStruct " << nVoxels << G4endl; //GDEB
...
//---------------------------------------------------------------------------
void DCMChangeExtraStructures::ChangeVoxelsPartialStruct()
{
...
//---------------------------------------------------------------------------
void DCMChangeExtraStructures::ChangeImagesByLimits()
{
  G4int nVoxels = theAnalyseImage->GetNoVoxels();
  ...
  //  G4cout << " DCMChangeExtraStructures::ChangeImagesByLimits " << nVoxels << G4endl; //GDEB
  for( G4int ii = 0; ii < nVoxels; ii++ ) {
...
//---------------------------------------------------------------------------
void DCMChangeExtraStructures::WriteNewImage()
{
  //--- DUMP TO FILE
...
//---------------------------------------------------------------------------
void DCMChangeExtraStructures::PrintHelp()
{
