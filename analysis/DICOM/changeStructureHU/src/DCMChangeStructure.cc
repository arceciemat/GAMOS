...
//---------------------------------------------------------------------------
void DCMChangeStructure::ChangeVoxelsPartialStruct()
{
  //------- LOOP TO VOXELS WITH A PERCENTAGE IN STRUCTURE AND CHANGE HU AND WITH THE NEW HU THEN DENSITY AND MATERIAL

  //--- LOOP TO VOXELS WITH A PERCENTAGE IN STRUCTURE 
  G4int nVoxels = theAnalyseImage->GetNoVoxels();
  //  G4cout << " DCMChangeStructure::ChangeImagesByStruct " << nVoxels << G4endl; //GDEB
  StructData stData = theStructData[0]; //GDEB
  //    G4cout << " STID SEARCHED " << stData.stID << G4endl; //GDEB
