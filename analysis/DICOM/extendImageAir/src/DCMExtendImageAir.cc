...
#include "DICOM/DICOMBase/include/DicomOperExtendAir.hh"
...
//---------------------------------------------------------------------------
DCMExtendImageAir::DCMExtendImageAir()
{
  theExeName = "resizeImage";
...
//---------------------------------------------------------------------------
void DCMExtendImageAir::ProcessArguments(int argc,char** argv)
{
  if(argc%2 != 1) {
...
//---------------------------------------------------------------------------
void DCMExtendImageAir::CheckArguments()
{
}
  
//---------------------------------------------------------------------------
void DCMExtendImageAir::ReadFilesAndGetImages()
{
  ...
  DicomOperExtendAir* operExtendAir = new DicomOperExtendAir();  
...
/*  imageReaders =  theReaderMgr->GetImageReaders(DRM_CT,false);
  for( size_t ii = 0; ii < imageReaders.size(); ii++ ) {    
    DicomReaderCT* readerCT = dynamic_cast<DicomReaderCT*>(imageReaders[ii]);
    if( fileName == "" ) fileName = readerCTNM->GetFileName()+".new";
  ...
  if( imageReaders.size() != 0 ) {
    G4Exception(theExeName, 
		"",
		FatalException,
		"DICOM CT image not implemented yet. Please ask the GAMOS team");
		}  */
  ...
  for( size_t ii = 0; ii < imageReaders.size(); ii++ ) {    
    ...
    DicomVImageStr* imageStr = 0;
    std::vector<DicomVReader*> RTStructReaders = theReaderMgr->GetReaders(DRM_RTStruct, false);
    G4cout << " N eReaders(DRM_RTStruct " << RTStructReaders.size() << G4endl; //GDEB
    if( RTStructReaders.size() == 0 ) {
      //--- G4StructID SET NEW VOXELS TO 0 OR REDO WITH RTSTRUCT
      imageStr = readerCT->GetStructIDImage();
      //-    image = readerCT->GetStructIDImage();
      operExtendAir->Operate(imageStr);
    } else {
    //-- REDO G4StructID WITH RTSTRUCT
      imageStr = new DicomVImageStr( image, "RTStruct", DIM_RTStruct );
...
//---------------------------------------------------------------------------
void DCMExtendImageAir::PrintHelp()
{
