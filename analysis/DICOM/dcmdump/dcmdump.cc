 ...
 *
 *  Last Update:      $Author: arce $
 *  Update Date:      $Date: 2020/03/18 09:20:47 $
...
static OFBool addPrintTagName(const char *tagName)
{
    ...
    unsigned int group = 0xffff;
    unsigned int elem = 0xffff;
    if (sscanf(tagName, "%x,%x", &group, &elem) != 2)   // --search ImagePositionPatient
    {
        /* it is a name */
        const DcmDataDictionary &globalDataDict = dcmDataDict.rdlock();
        const DcmDictEntry *dicent = globalDataDict.findEntry(tagName);
        ...
        dcmDataDict.rdunlock();
    } else {                                             // --search 0020,0032
        /* tag name has format xxxx,xxxx */
        /* do not lookup in dictionary, tag could be private */
        printTagKeys[printTagCount] = NULL;
    }

    COUT << printTagCount << " addPrintTagName printTagKeys " << tagName << " " <<  printTagKeys[printTagCount] << OFendl; //GDEB
	   
    printTagNames[printTagCount] = strcpy(OFstatic_cast(char*, malloc(strlen(tagName)+1)), tagName);
       COUT << printTagCount << " addPrintTagName " << tagName << " " <<  printTagNames[printTagCount] << OFendl; //GDEB
    printTagCount++;
    return OFTrue;
}
...
static int dumpFile(STD_NAMESPACE ostream &out,
                    ...
                    const size_t printFlags,  // DCMTypes::PF_shortenLongTagValues; --print-tree
                    const OFBool loadIntoMemory, // OFTrue
                    const OFBool stopOnErrors, // OFTrue
                    const char *pixelDirectory)  // NULL
{
    int result = 0;
    ...
    if (pixelDirectory != NULL) // --write-pixel
    {
	...
	//	out << " PIXEL FILE NAME " << pixelFileName << OFendl; //GDEB
    }

    /* dump complete file content */
    if (printTagCount == 0) // if not  --search  full file is printed
    {
      dset->print(out, printFlags, 0 /*level*/, pixelFileName, &pixelCounter);
      std::cout << " pixelCounter " << pixelCounter << " printFlags " << printFlags << " pixelFileName " << pixelFileName << std::endl;//GDEB
    } else {
	    ...
	    COUT << i <<  " printTagNames " << printTagNames[i] << " printTagKeys " <<printTagKeys[i] << OFendl; //GDEB 
            if (printTagKeys[i]) {  // search by name
                searchKey = *printTagKeys[i];
		COUT << i <<  " searchKey FOUND  " << searchKey <<  OFendl;//GDEB
            } else if (sscanf(tagName, "%x,%x", &group, &elem) == 2) {	      
                searchKey.set(group, elem);
		COUT << i <<  " searchKey  " << searchKey << " tagName "<< tagName << " group " << group << " elem " << elem <<  OFendl;//GDEB
            } else {
		...
		std::cout << "  printResult " << pixelFileName << std::endl;//GDEB
		printResult(out, stack, printFlags, pixelFileName, &pixelCounter);
		  ...
		  while (dset->search(searchKey, stack, ESM_afterStackTop, OFTrue) == EC_Normal) {
		    std::cout << "  printResult2 " << pixelFileName << std::endl;//GDEB
		    printResult(out, stack, printFlags, pixelFileName, &pixelCounter);
		  }
                }
            }
 ...
 * CVS/RCS Log:
 * $Log: dcmdump.cc,v $
 * Revision 1.14  2020/03/18 09:20:47  arce
 * *** empty log message ***
 *
 * Revision 1.13  2020/03/18 08:43:32  arce
 * *** empty log message ***
 *
 * Revision 1.6  2018/07/27 19:25:15  arce
 * *** empty log message ***
 *
 * Revision 1.5  2018/06/07 21:25:35  arce
 * *** empty log message ***
 *
 * Revision 1.4  2018/06/01 16:44:20  arce
 * *** empty log message ***
 *
 * Revision 1.1  2017/12/10 22:22:08  arce
 * *** empty log message ***
 *
 * Revision 1.2  2016/07/28 09:13:15  arce
 * *** empty log message ***
 *
 * Revision 1.1  2016/06/29 09:59:33  arce
 * *** empty log message ***
 *
