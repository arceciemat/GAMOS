// @(#)root/io:$Id: TFileMerger.cc,v 1.8 2020/10/04 21:01:31 arce Exp $
...
Bool_t TFileMerger::PartialMerge(Int_t in_type)
{
   ...
   Bool_t result = kTRUE;
   Int_t type = in_type;
   int nFiles = fFileList->GetEntries();
   ...
   if (!result) {
     ...
     //--- GAMOS: DIVIDE BY NUMBER OF FILES
     TIter ListOfHistograms( fOutputFile->GetListOfKeys() );
     TKey *HistoKey = 0;
     //     std::cout << " DIVIDE /NFILES " << nFiles << " " << std::endl; //GDEB
     while ( (HistoKey = (TKey*)ListOfHistograms()) ) { 
       std::string hisname = HistoKey->GetName();
       TH1* histo = (TH1*) fOutputFile->Get( HistoKey->GetName() );
	 
       if( histo->GetDimension() == 1 ) {
	 TH1F*  histo1 = (TH1F*) fOutputFile->Get( HistoKey->GetName() );
	 *histo1 = *histo1 * (1./nFiles);
	 //	 std::cout << " /NFILES " << nFiles << " " << histo1->GetName() << std::endl; //GDEB
	 fOutputFile->Append(histo1, kTRUE);
	 //histo = histo1;
	 for( size_t ii = 0; ii < histo->GetXaxis()->GetNbins(); ii++ ) {
	   if( histo->GetBinContent(ii) != 0 ) {
	     std::cout << ii << " " << histo->GetName() << histo->GetBinContent(ii) << std::endl; //GDEB
	   }
	 }
	  
       } else if( histo->GetDimension() == 2 ) {
	 TH2F*  histo2 = (TH2F*) fOutputFile->Get( HistoKey->GetName() );
	 *histo2 = *histo2 * (1./nFiles);
	 fOutputFile->Append(histo2, kTRUE);
       }
       //     *histo = *histo * (1./nFiles);
     }
