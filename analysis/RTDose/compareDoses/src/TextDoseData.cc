...
//----------------------------------------------------------------------
TextDoseData::TextDoseData(std::vector<G4String> wl) 
  : DoseData()
{
  theName = wl[0];
  ...
  //  if( TextDoseData::verbosity >= 2 )
  //   G4cout << " TextDoseData " << this << " " << theName << G4endl;
  ...
  if( TextDoseData::verbosity >= 2 )
    G4cout << " TextDoseData " << theName << " : " << theDose << " +- " << theDoseError << " DivideError " << theDoseDivideError << " InvErrorExp2 " << theInvErrorExp2 << G4endl;
...
//----------------------------------------------------------------------
void TextDoseData::Print( std::ofstream& fout )
{
  TextDoserData* theDoserDataG = static_cast<TextDoserData*>(theDoserData);
  ...
  if( TextDoseData::verbosity >= 2 ) 
    G4cout << "   TextDoseData::Print : " << theName << " doseDisp " << doseDisp << " N " << nFiles << " dose " << theDose << " doseSq " << theDoseSq << " theDoseSq/nFiles - sqr(theDose/nFiles) " << theDoseSq/nFiles - sqr(theDose/nFiles) << G4endl;
