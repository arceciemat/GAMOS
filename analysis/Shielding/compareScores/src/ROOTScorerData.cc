...
//----------------------------------------------------------------------
ROOTScorerData::ROOTScorerData(TH1F* histo) : ScorerData()
{
  theDoseType = ST_ROOT;

  TAxis* xAxis = histo->GetXaxis();
  for( int ii = 1; ii < xAxis->GetNbins(); ii++ ) {
    //t    ROOTScoreData* doseData = new ROOTScoreData(histo->GetBinCenter(ii),histo->GetBinContent(ii), histo->GetBinError(ii));
    ROOTScoreData* doseData = new ROOTScoreData(histo->GetBinCenter(ii),histo->GetBinContent(ii), 0);
      
    AddScoreData( doseData );
    SetNDoses( GetNScores() + 1 );
    if( ScoreData::verbosity >= 2 ) G4cout << "@@@ New ScoreData N ENTRIES " << GetNScores() << G4endl; 
  }
  
}

//----------------------------------------------------------------------
void ROOTScorerData::Print( std::ofstream& fout )
{
  fout << " Doser: " << theName << G4endl;
  fout << " Number of entries= " << theNDoses << G4endl;
  
  if( ScoreData::verbosity >= 2 ) G4cout << "  ROOTScorerData::Print: " << theName << G4endl; 
...
//----------------------------------------------------------------------
G4double ROOTScorerData::GetIndexVal( G4String doseName, G4bool bLogX ) 
{
  G4double indexVal = 0.;
  //--- Find separating '-' 
