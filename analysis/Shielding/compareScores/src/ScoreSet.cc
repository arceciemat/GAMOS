...
//-----------------------------------------------------------------------------
ScoreSet::ScoreSet(std::vector<G4String> wl, G4bool bLogXDef, G4bool bLogYDef  )
{
 if( wl.size() != 3 &&
      wl.size() != 4 && 
      wl.size() != 6 ) {
...
//-----------------------------------------------------------------------------
void ScoreSet::AddScorer(G4String fileName, G4String doseName, G4double multFactor, G4double dispFactor, G4int iColor, G4double dispYFactor)
{
  ScoreInfo doseInfo;
  ...
  theScoreInfos.push_back( doseInfo );
