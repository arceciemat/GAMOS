...
void PlotScorers( ScoreSet* doseSet, G4bool bLogX );
ScorerData* FindScorerData( G4String fileName , G4String doserName );
...
int main(int argc,char** argv) 
{
  ...
  if( ScoreData::verbosity >= 1) G4cout << "@@@@@@@ READING COMPARE META FILE: " << theCompareFileName << G4endl;
  ReadCompareFile( theCompareFileName );

  std::set<pss>::const_iterator itef;
  for( itef = theFiles.begin(); itef != theFiles.end(); itef++ ) {
    if( ScoreData::verbosity >= 1) G4cout << "@@@@@@@ READING FILE: " << (*itef).first << " of type " << (*itef).second << G4endl;
  ...
  for( itess = theScoreSets.begin(); itess != theScoreSets.end(); itess++ )  {
    if( ScoreData::verbosity >= 1) G4cout << "@@@@@@@ PLOT DOSERS: " << (*itess)->GetHisFileName() << G4endl;
    //    G4cout << " CALL PLOT DATA " << bLogX << G4endl;//GDEB
    PlotScorers(*itess,bLogX);
  }
...
void ReadCompareFile( G4String fileName )
{
  ...
  for( G4int ii = 0;; ii++) {
    ...
    if( wl[0] == "COMPARE" ) {
      doseSet = new ScoreSet(wl, Definitions::bLogX, Definitions::bLogY);
      theScoreSets.push_back(doseSet);
    } else {
      if( doseSet == 0 ) {
	...
	G4Exception("ReadCompareFile",
      ...
      doseSet->AddScorer(fName,doseName,multFactor,dispFactor, iColor, dispYFactor);
...
void PlotScorers( ScoreSet* doseSet, G4bool bLogX  ) 
{
  ...
  Definitions::yAxisTitle = GetDrawingDataString("yAxisTitle","Score");
  ...
  //----- ScoreSet IS THE SET OF DOSERS TO COMPARE, WHICH START BY LINE
  ...
  //----- ScoreInfo IS THE DOSE INFORMATION OF EACH DOSER:
  ...
  std::vector<ScoreInfo> fScoreInfos = doseSet->GetScoreInfos();
  G4bool bNorm = doseSet->IsNormalized();
  
  vpsmdpdd plotScorers;
  std::map<G4String,G4int> theIdxColors;

  //----- TRANSFORM EACH ScoreInfo TO PLOTTING FORMAT :
  // msmdpdd plotScorers[(*itess).fileName+"_"+(*itess).doseName] = doserValues; 
  // mdpdd doserValues : X_val std::pair<Y_val,Y_err>
  std::vector<ScoreInfo>::const_iterator itess;
  G4bool bEmpty = true;
  for( itess = fScoreInfos.begin(); itess != fScoreInfos.end(); itess++ ) {
    ScorerData* doserData = FindScorerData( (*itess).fileName, (*itess).doseName );
    if( doserData->GetNScores() != 0 ) bEmpty = false;
    if( ScoreData::verbosity >= 1) G4cout << "@@@@@ PLOT DOSE INFO: " << (*itess).fileName << " " << (*itess).doseName << " N_DOSES " << doserData->GetNScores() << G4endl;
	...
	for( ite = doserValues->begin(); ite != doserValues->end(); ite++ ) {
      ...
      plotScorers.push_back(psmdpdd((*itess).fileName+"_"+(*itess).doseName, doserValues));
      if( ScoreData::verbosity >= 2) G4cout << "@@ FILLED doserValues (mdpdd) " << (*itess).fileName+"_"+(*itess).doseName << " N " << doserValues->size() << G4endl;
  ...
  if( bEmpty ) {
    G4cerr << " compareScores: all doses are empty, exiting " << G4endl;
    return;
  }
  ...
  if( bNorm ) {
    for( itecs = plotScorers.begin(); itecs != plotScorers.end(); itecs++ ) {
      G4double sumScore = 0.;
      mdpdd* data1 = (*itecs).second;
      for( itec = data1->begin(); itec != data1->end(); itec++ ) {
	sumScore += (*itec).second.first;
      }
      for( itec = data1->begin(); itec != data1->end(); itec++ ) {
	(*itec).second.first /= sumScore;
	(*itec).second.second /= sumScore;
      }
      if( ScoreData::verbosity >= 1) G4cout << "@@ data NORMALIZED " << (*itecs).first << " sumScore= " << sumScore << G4endl;
    }
  }
 
  //----- LOOP TO plotScorers TO PLOT ONE HISTOGRAM PER DOSER
  G4int ipd = 0;
  ...
  if( ScoreData::verbosity >= 1) G4cout << "@@ Data sets histo limits? " << bThereIsData << G4endl;
  //--- IF THERE IS DATA ONLY USE DATA, ELSE USE ALL
  G4int nPlots = plotScorers.size();
  for( itecs = plotScorers.begin(); itecs != plotScorers.end(); itecs++) {
    mdpdd* data1 = (*itecs).second;
    G4bool bIsFirst = (itecs == plotScorers.begin());
    // Data SETS THE HISTO LIMITS, NOT MC
  ...
  for( itecs = plotScorers.begin(); itecs != plotScorers.end(); itecs++, ipd++ ) {
    ...
    if( bIsFirst ) {
      ...
      std::cout << "  DIV " << plotScorers.size() << " " << doseSet->PlotDivided() << G4endl; //GDEB
      TPad* pad1;
      if( plotScorers.size() == 2 && doseSet->PlotDivided() ) {
	//       TPad* pad1 = new TPad("","",0.,0.,1.,0.25);
	pad1 = new TPad("","",Definitions::padLeftMargin,0.25,1.-Definitions::padRightMargin,1.);
	std::cout << " APD DIV " << Definitions::padLeftMargin << " 0.25 " << 1.-Definitions::padRightMargin << " 1. " << G4endl; //GDEB
    ...
    if( ScoreData::verbosity >= 2) G4cout << "@@@@@ DrawHisto: N " << data1->size() << G4endl;
  ...
  if( bStatComp ) {
    ...
    EColor color;
    ipd = 0;
    for( itecs = plotScorers.begin(); itecs != plotScorers.end(); itecs += 2, ipd += 2) {
      itecs2 = itecs; 
      itecs2++;
      mdpdd* data1 = (*itecs).second;
      mdpdd* data2 = (*itecs2).second;
      if( ScoreData::verbosity >= 1) G4cout << "@@@@@@@ START STATISTICAL COMPARISON: " <<(*itecs).first << " vs. " << (*itecs2).first << G4endl;
	...
	if( ScoreData::verbosity >= 1) G4cout << "@@@@ CHI2 p-value= " << cs.pValue << " CHI2: " << GmGenUtils::ftoa(cs.chi2)+"/"+GmGenUtils::ftoa(cs.ndof) << " " << hisFileName << G4endl;
      }    
	...
	if( setName.find("Prof") != std::string::npos ) {
	  ...
	  if( ScoreData::verbosity >= 3) G4cout << "@@@@ Field to define gamma index limits " << fieldStr << " = " << field << " from " << setName << " ip1 " << ip1 << " ip2 " << ip2 << G4endl;
	  theGMinX = -field*8.;
	  theGMaxX = -theGMinX;	
	} else {
	  // PDD CENTRAL LIMITS
	...
	for( mdpdd::const_iterator itedd = dataDiv->begin(); itedd != dataDiv->end(); itedd++ ) {
	  G4cout << " DATADIV " << itedd->first << " : " << itedd->second.first << " +- " << itedd->second.second << G4endl; //GDEB
	}
...
ScorerData* FindScorerData( G4String fileName , G4String doserName )
{
  ...
  if( doserData == 0 ) {
    ...
    for( itefd = theFileData.begin(); itefd != theFileData.end(); itefd++ ) {
      FileData* fileData = (*itefd);
     G4cerr << "@@  FILE: " << fileData->GetFileName() <<  " N DOSES " << fileData->GetNumberOfScorers() << G4endl;
     size_t nsco = fileData->GetNumberOfScorers();
      for( size_t ii = 0; ii < nsco; ii++ ) {
	ScorerData* sd = fileData->GetScorerData(ii);
    ...
    G4Exception("FindScorerData",
		...
		//		JustWarning,
		FatalException,
		(G4String("Scorer not found " + doserName + " in file " + fileName).c_str()));

  } else {
  
    if( ScoreData::verbosity >= 2) G4cout << " FindScorerData " << fileName << " " << doserName << " =N " << doserData->GetNScores() << G4endl;
  }
  return doserData;
}
...
void SetUserHistoLimits()
{
  ...
  if( ScoreData::verbosity >= 1) G4cout << "@@@@@ SetUserHistoLimits: " << theHXMin << " < X < " << theHXMax << " " << theHYMin << " < Y < " << theHYMax << G4endl;
