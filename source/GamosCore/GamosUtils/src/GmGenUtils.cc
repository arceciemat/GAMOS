...
//------------------------------------------------------------------------
G4bool GmGenUtils::IsNumber( char ch )
{
  return IsNumber(G4String(1,ch));
}
...
//------------------------------------------------------------------------
G4bool GmGenUtils::IsInteger( char ch ) 
{
  return IsInt( G4String(1,ch) );
}
...
std::string::size_type GmGenUtils::GetNextSeparator( G4int iSeparator, G4String dataName )
{
  ...
  for( G4int ii = 0; ii < 6; ii++ ){
    ...
    if( ii < 2 && G4int(isepFt) >= 2 ) {
	  ...
	  && ( GmGenUtils::IsNumber( std::string(1,dataName[isepFt-2]) ) || dataName[isepFt-2] == '.' )
	  && ( GmGenUtils::IsNumber( std::string(1,dataName[isepFt+1]) ) ) ){
