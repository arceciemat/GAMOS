...
//-----------------------------------------------------------------------
GmVPhysicsElectron::GmVPhysicsElectron(const G4String& name): G4VPhysicsConstructor(name)
{
}
...
{
  G4String angularDist = GmParameterMgr::GetInstance()->GetStringValue("GmPhysicsElectron:Bremsstrahlung:AngularDistribution","tsai");
  if( angularDist == "tsai" ) {
    bremsModel->SetAngularDistribution( new G4ModifiedTsai() );
  }else if( angularDist == "2bn" ) {
    bremsModel->SetAngularDistribution( new G4Generator2BN() );
