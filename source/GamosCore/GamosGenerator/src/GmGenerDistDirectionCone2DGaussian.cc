...
//---------------------------------------------------------------------
G4ThreeVector GmGenerDistDirectionCone2DGaussian::GenerateDirection( GmParticleSource* )
{
#ifndef GAMOS_NO_VERBOSE
  if( GenerVerb(debugVerb) ) G4cout << " GmGenerDistDirectionCone2DGaussian::Generate:  initial dir " << theAxisDir << " opening angleX (deg) " << sqrt(theSigmaOpeningAngleX2)/CLHEP::deg << " opening angleY (deg) " << sqrt(theSigmaOpeningAngleY2)/CLHEP::deg << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
  if( GenerVerb(infoVerb) ) G4cout << " GmGenerDistDirectionCone2DGaussian::SetParams theAxisDir " << theAxisDir << "thePerpDir " << thePerpDir << " opening angle X " << sqrt(theSigmaOpeningAngleX2) << " angle Y " << sqrt(theSigmaOpeningAngleY2) << G4endl;
#endif
