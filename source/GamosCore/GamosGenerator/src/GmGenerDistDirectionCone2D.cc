...
//---------------------------------------------------------------------
G4ThreeVector GmGenerDistDirectionCone2D::GenerateDirection( GmParticleSource* )
{
#ifndef GAMOS_NO_VERBOSE
  if( GenerVerb(debugVerb) ) G4cout << " GmGenerDistDirectionCone2D::Generate:  initial dir " << theAxisDir << " opening angleX (deg) " << sqrt(theOpeningAngleX2)/CLHEP::deg << " opening angleY (deg) " << sqrt(theOpeningAngleY2)/CLHEP::deg << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
  if( GenerVerb(infoVerb) ) G4cout << " GmGenerDistDirectionCone2D::SetParams theAxisDir " << theAxisDir << "thePerpDir " << thePerpDir << " opening angle X " << sqrt(theOpeningAngleX2)/CLHEP::deg << " Y " << sqrt(theOpeningAngleY2)/CLHEP::deg << G4endl;
#endif
