...
//-----------------------------------------------------------------------
RTPlanPSSource::RTPlanPSSource( const G4String& name, const G4String& partName): RTVPlanSource( name )
{
  theType = "RTPlanPSSource";
...
//---------------------------------------------------------------------
void RTPlanPSSource::ReadPlanData()
{
  GmFileIn fin = GmFileIn::GetInstance(thePlanFileName,1);
  ReadParams(fin);

  G4cout << "RTPlanPSSource::ReadPlanData() N PARAM " << theParams.size() << G4endl; //GDEB
  ...
  if( fracGroups.size() == 0 ) {
    G4Exception("RTPlanPSSource::ReadPlanData",
		"",
		FatalException,
  ...
  for( std::set<int>::iterator ite = fracGroups.begin(); ite != fracGroups.end(); ite++ ) {
    ...
    //      G4cout << "RTPlanPSSource::ReadPlanData() nBeams " << nBeams << G4endl; //GDEB      
	...
	G4Exception("RTPlanPSSource::RTPlanPSSource",
		    "",
		    JustWarning,
...
/*//-------------------------------------------------------------
G4double RTPlanPSSource::GetSpotSize( RTBeamStateData )
{
  if( !bDiscGaussian ) {
    return theDiscSigma;
  } else {
...
//-------------------------------------------------------------
void RTPlanPSSource::SetSpotSize( GmVGenerDistPosition2D* , const RTBeamStateData& )
{
  return;
...
//-------------------------------------------------------------
void RTPlanPSSource::PlotBeamStates()
{
  return;
