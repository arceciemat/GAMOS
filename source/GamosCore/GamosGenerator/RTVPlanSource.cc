...
//  bCoordSystemPatient = G4bool(theParamMgr->GetNumericValue(theName+":CoordinateSystemPatient",1));
  ...
  bCorrectPosDir = G4bool(theParamMgr->GetNumericValue(theName+":CorrectPosDir",0.));
...
//-----------------------------------------------------------------------
void RTVPlanSource::ReadEnerDistF()
{
  ...
  for( ;; ){
    ...
    //    G4cout << " RTVPlanSource::ReadEnerDistF( FILL " << wl[1] << " = " << wl[0] << G4endl; //GDEB
  }
}


//-----------------------------------------------------------------------
void RTVPlanSource::ReadCorrectPosDirExpMCProfAir1D(G4String energyNominal)
{
...
//-----------------------------------------------------------------------
G4bool RTVPlanSource::CorrectPosDir()
{
  ...
  if( energyDistFMF ) {
    ...
    if( itee == theCorrectPosDirEnergyDistFiles.end() ) {
      G4Exception("RTVPlanSource CorrectPosDir",
		  "",
		  FatalException,
		  ("no energy distribution file for energy "+energyFileName).c_str());
    }
    G4String energyNominal = itee->second;
...
// Extrapolate position to isocenter plane (at 0,0,0)
    ...
    G4double corrFactorX = posDirCorrDistX->GetValue(posR2*std::copysign(1.0, posX),bOutOfRange) * 0.8; // must be smaller than 1, if not
    ...
    G4double corrFactorY = posDirCorrDistY->GetValue(posR2*std::copysign(1.0, posY),bOutOfRange) * 0.8;
    ...
    //    G4cout << " RTVPlanSource::CorrectPosDir OK " << posX << " " << posY << G4endl; //GDEB
