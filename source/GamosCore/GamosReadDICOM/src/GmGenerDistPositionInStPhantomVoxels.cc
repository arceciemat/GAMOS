...
//------------------------------------------------------------------------
GmGenerDistPositionInStPhantomVoxels::GmGenerDistPositionInStPhantomVoxels()
{
...
//------------------------------------------------------------------------
void GmGenerDistPositionInStPhantomVoxels::BuildVoxelPositions()
{
  ...
  for( ite = pvstore->begin(); ite != pvstore->end(); ite++ ) {
	...
	if( ReadDICOMVerb(infoVerb) ) G4cout << "GmGenerDistPositionInStPhantomVoxels volume found " << (*ite)->GetName() << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
	if( ReadDICOMVerb(infoVerb) ) G4cout << "GmGenerDistPositionInStPhantomVoxels container touchable " << *containerTouch << G4endl;
#endif
	
	G4VPhysicalVolume* containerPV = geomUtils->GetPhysicalVolumes( (*ite)->GetMotherLogical()->GetName() )[0];
#ifndef GAMOS_NO_VERBOSE
	if( ReadDICOMVerb(infoVerb) ) G4cout << "GmGenerDistPositionInStPhantomVoxels container physvol " << containerPV->GetTranslation() << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
	if( ReadDICOMVerb(infoVerb) ) G4cout << "GmGenerDistPositionInStPhantomVoxels voxel dimensions " << theVoxelDimX << " " << theVoxelDimY << " " << theVoxelDimZ << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
	if( ReadDICOMVerb(infoVerb) ) G4cout << "GmGenerDistPositionInStPhantomVoxels number of voxels " << theNoVoxelsX << " " << theNoVoxelsY << " " << theNoVoxelsZ << G4endl;
#endif
  ...
  if( !thePhantomParam ) {
    G4Exception("GmGenerDistPositionInStPhantomVoxels::SetParams",
		"Wrong distirbution",
  ...
  filterNames = GmParameterMgr::GetInstance()->GetVStringValue("GmGenerDistPositionInStPhantomVoxels:Filters",filterNames);
  ...
  for( G4int iz = 0; iz < theNoVoxelsZ; iz++ ) { 
	...
	G4int copyNo = ix + thePhantomParam->GetNoVoxelsX()*iy + thePhantomParam->GetNoVoxelsX()*thePhantomParam->GetNoVoxelsY()*iz;
	...
	//	G4cout << "GmGenerDistPositionInStPhantomVoxels CHECK INDEX " << copyNo << " stid: " << stids[0] << " bInSt= " << bInSt << G4endl; //GDEB
	if( !bInSt ) continue;
...
#ifndef GAMOS_NO_VERBOSE
	  if( ReadDICOMVerb(infoVerb) ) G4cout << "GmGenerDistPositionInStPhantomVoxels voxel pass " << copyNo << " pos " << pos << " NVOXELS " << theVoxelPositions.size() << G4endl;
#endif
	}
      }
    }
...
#ifndef GAMOS_NO_VERBOSE
  if( ReadDICOMVerb(infoVerb) ) G4cout << "GmGenerDistPositionInStPhantomVoxels::GeneratePosition voxelID " << voxelID << " pos " << pos << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
  if( ReadDICOMVerb(infoVerb) ) G4cout << "GmGenerDistPositionInStPhantomVoxels::GeneratePosition  pos FINAL " << pos << G4endl;
#endif
...
//---------------------------------------------------------------------
void GmGenerDistPositionInStPhantomVoxels::SetParams( const std::vector<G4String>& params )
{
  if( ReadDICOMVerb(debugVerb) ) G4cout << "GmGenerDistPositionInStPhantomVoxels::SetParams" << G4endl;
  if( params.size() < 1 ) {
    G4Exception(" GmGenerDistPositionInStPhantomVoxels::SetParams",
		"Wrong argument",
		FatalErrorInArgument,
  ...
  for( size_t ii = 0; ii < params.size(); ii++ ) {
    std::vector<G4int> stids = theReadPhantomStMgr->GetStIDFromPhysVolName( params[ii] );
    for( size_t ii1 = 0; ii1 < stids.size(); ii1++ ) {
      theStIDs.insert(stids[ii1]);
#ifndef GAMOS_NO_VERBOSE
      if( ReadDICOMVerb(debugVerb) ) G4cout << "GmGenerDistPositionInStPhantomVoxels StID included " << stids[ii1] << " from " << params[ii] << G4endl;
#endif
    }
  }
