...
#endif
  if( !the1stHitAlgo1 ) { 
    G4Exception("PETEventClassifierUA::PETEventClassifierUA",
...
//----------------------------------------------------------------------
void PETEventClassifierUA::BookHistos()
{
...
//----------------------------------------------------------------------
void PETEventClassifierUA::BeginOfRunAction(const G4Run* )
{
...
//----------------------------------------------------------------------
int64_t PETEventClassifierUA::Classify( gamosRecHitList& rhitList, const G4Event* evt  )
{
...
//----------------------------------------------------------------------
int64_t PETEventClassifierUA::ClassifyPET( gamosRecHitList& rhitList, const G4Event* evt )
{
  the511RecHits.clear();

#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(infoVerb) ) G4cout << " PETEventClassifierUA::ClassifyPET() n rechits " << rhitList.size() << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << " PETEventClassifierUA::ClassifyPET  creating line " << the511RecHits[0].GetPosition() << " " << the511RecHits[1].GetPosition() << G4endl;
#endif
    theEventClass += 1;
    G4double linedist;
...
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(infoVerb) ) G4cout << " PETEventClassifierUA: classification " << theEventClass << G4endl;
#endif
...
//----------------------------------------------------------------------
void PETEventClassifierUA::Get511ComptonRecHits( const gamosRecHitList& rhitList )
{
...
#ifndef GAMOS_NO_VERBOSE
	if( PETVerb(debugVerb) ) G4cout <<"PETEventClassifierUA::Get511ComptonRecHits: insert first not used Rechit in set " << seedRecHit->GetPosition() << " E " << seedRecHit->GetEnergy() << G4endl;
#endif
	break;
      }
    }
	...
	if( recHitUsed.find( newRecHit ) == recHitUsed.end() && 
	  ...
	  if( dist < theComptonRecHitsDist ){
	    ...
	    if( PETVerb(debugVerb) ) G4cout 
	      << "PETEventClassifierUA::Get511ComptonRecHits: " 
...
#ifndef GAMOS_NO_VERBOSE
      if( PETVerb(debugVerb) ) G4cout 
	     << "PETEventClassifierUA::Get511ComptonRecHits: rec hit set Energy BAD " 
...
#ifndef GAMOS_NO_VERBOSE
      if( PETVerb(debugVerb) ) G4cout 
	     << "PETEventClassifierUA::Get511ComptonRecHits: rec hit set Energy GOOD " 
...
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout 
	 << "PETEventClassifierUA::Get511ComptonRecHits: building of rec hit sets ended. N sets built = " 
...
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierUA::Get511ComptonRecHits: deleting rec hit set " << itedss->second.GetEnergy() << G4endl;
#endif
  ...
  G4ThreeVector pos;
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierUA::Get511ComptonRecHits: firstRecHitSet initial pos " << firstRecHitSet.GetPosition() << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierUA::Get511ComptonRecHits: firstRecHitSet change pos to " << pos << " biggest E  " << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierUA::Get511ComptonRecHits: firstRecHitSet  pos final " << firstRecHitSet.GetPosition() << G4endl;
#endif
  ...
  if( distEnerComptRecHitList.size() == 2 ) {
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierUA::Get511ComptonRecHits: secondRecHitSet initial pos " << secondRecHitSet.GetPosition() << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
      if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierUA::Get511ComptonRecHits: secondRecHitSet change pos " << pos << G4endl;
#endif
      secondRecHitSet.SetPosition(pos);
      if( PETVerb(debugVerb) ) secondRecHitSet.Print();
    }
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierUA::Get511ComptonRecHits: secondRecHitSet pos final " << secondRecHitSet.GetPosition() << G4endl;
#endif
    //--- Register it as 511 rec hit
...
//----------------------------------------------------------------------
G4bool PETEventClassifierUA::CheckTimeCoincidence( mddrh::iterator itedss1, mddrh::iterator itedss2 )
{
...
//----------------------------------------------------------------------
void PETEventClassifierUA::CheckTimeCoincidence()
{
...
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << " PETEventClassifierUA::CheckTimeCoincidence() " << (*ites1).GetTime() - (*ites2).GetTime() << " >? " << theCoincidenceTime << " time1= " << (*ites1).GetTime() << " time2= " << (*ites2).GetTime() << G4endl;
#endif
...
//----------------------------------------------------------------------
G4ThreeVector PETEventClassifierUA::GetEventVertex( const G4Event* evt )
{
...
//----------------------------------------------------------------------
G4double PETEventClassifierUA::GetPETLineDistanceToVtx( const GmLine& line, const G4ThreeVector& eventVtx )
{
  if( the511RecHits.size() != 2 ) {
    G4Exception("PETEventClassifierUA::GetPETLineDistanceToVtx",
		"Error",
		FatalException,
		G4String("Number of 511 recHits !=2 " + GmGenUtils::itoa( the511RecHits.size() ) ).c_str());
  }

  //  if( !foundPositron ) G4Exception("PETEventClassifierUA::GetPETLineDistanceToVtx: no positron found ");

#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << " PETEventClassifierUA::GetPETLineDistanceToVtx recHit 0 pos " << the511RecHits[0].GetPosition()<< " E " << the511RecHits[0].GetEnergy()<< " recHit 1 pos " << the511RecHits[1].GetPosition()<< " E " << the511RecHits[1].GetEnergy() << G4endl;
#endif

  G4double linedist = line.GetDist( eventVtx );
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(infoVerb) ) G4cout << " PETEventClassifierUA::GetPETLineDistanceToVtx " << linedist << " ?< " << thePETLineDistToVtx << G4endl;
#endif
...
//----------------------------------------------------------------------
void PETEventClassifierUA::FillDCAHistos( G4double linedist )
{
...
//----------------------------------------------------------------------
G4bool PETEventClassifierUA::CheckDifferentOriginalGammaAndEvent( DetRecHitSetList& rhitsets )
{
...
//----------------------------------------------------------------------
G4bool PETEventClassifierUA::CheckRecHitDifferentOriginalGammaAndEvent( GmRecHit* rhit, G4int& origGammaAndEvent )
{
...
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << origGammaAndEvent << "PETEventClassifierUA::CheckRecHitDifferentOriginalGammaAndEvent hits in recHit " << hits.size() << G4endl;
#endif
  //  G4double maxdist = 0.;
...
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout 
	   << "PETEventClassifierUA::CheckRecHitDifferentOriginalGammaAndEvent looping hits n tracks " 
	   << htidori.size() << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
	if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierUA::CheckRecHitDifferentOriginalGammaAndEvent origGammaAndEvent changed " << origGammaAndEvent << G4endl;
#endif
	return isDifferentG;
      }
    }
...
//----------------------------------------------------------------------
G4bool PETEventClassifierUA::CheckOriginalGammaScattered( DetRecHitSetList& rhitsets )
{
  //----- Loop to all recHits original gammas to check 
  DetRecHitSetList::iterator ites;
  std::set<GmRecHit*>::iterator ite;
...
//----------------------------------------------------------------------
G4bool PETEventClassifierUA::CheckRecHitOriginalGammaScattered( GmRecHit* rhit )
{
  if( rhit == 0 ) return FALSE;
  ...
  std::set<G4int>::iterator itesi;
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierUA::CheckRecHitOriginalGammaScattered hits in recHit " << hits.size() << G4endl;
#endif

  for( iteh = hits.begin(); iteh != hits.end(); iteh++ ){
    std::set<G4int> htidori = (*iteh)->GetOriginalTrackIDs();
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierUA::CheckRecHitOriginalGammaScattered looping hits n original tracks " << htidori.size() << G4endl;
#endif
    for( itesi = htidori.begin(); itesi != htidori.end(); itesi++ ){
#ifndef GAMOS_NO_VERBOSE
...
//----------------------------------------------------------------------
void PETEventClassifierUA::EndOfRunAction(const G4Run* aRun )
{
  ...
  if( bDumpLM ) PETIOMgr::GetInstance()->CloseFileOut();
...
//----------------------------------------------------------------------
void PETEventClassifierUA::PrintPETClassificationTable( int64_t nevt )
{
  theAnaMgr = GmAnalysisMgr::GetInstance("pet") ;
...
//----------------------------------------------------------------------
void PETEventClassifierUA::EndOfEventAction(const G4Event* evt )
{
