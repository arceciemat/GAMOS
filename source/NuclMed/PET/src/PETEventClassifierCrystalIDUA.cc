...
//----------------------------------------------------------------------
PETEventClassifierCrystalIDUA::PETEventClassifierCrystalIDUA()
{
  bDumpLM = G4bool(GmParameterMgr::GetInstance()->GetNumericValue("PET:EvtClass:DumpEventListMode",0));
  if( bDumpLM ) PETIOMgrCrystalID::GetInstance()->OpenFileOut();
...
#endif
  if( !the1stHitAlgo1 ) { 
    G4Exception("PETEventClassifierCrystalIDUA::PETEventClassifierCrystalIDUA",
		("Wrong 1st hit algorithm 1= "+the1stHit1Str).c_str(),
  ...
  if( !the1stHitAlgo2 ) { 
    G4Exception("PETEventClassifierCrystalIDUA::PETEventClassifierCrystalIDUA",
		("Wrong 1st hit algorithm 2= "+the1stHit2Str).c_str(),
		FatalErrorInArgument,
...
//----------------------------------------------------------------------
void PETEventClassifierCrystalIDUA::BookHistos()
{
...
//----------------------------------------------------------------------
void PETEventClassifierCrystalIDUA::BeginOfRunAction(const G4Run* )
{
...
//----------------------------------------------------------------------
int64_t PETEventClassifierCrystalIDUA::Classify( gamosRecHitList& rhitList, const G4Event* evt  )
{
...
//----------------------------------------------------------------------
int64_t PETEventClassifierCrystalIDUA::ClassifyPET( gamosRecHitList& rhitList, const G4Event* evt )
{
  the511RecHits.clear();

#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(infoVerb) ) G4cout << " PETEventClassifierCrystalIDUA::ClassifyPET() n rechits " << rhitList.size() << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << " PETEventClassifierCrystalIDUA::ClassifyPET  creating line " << the511RecHits[0].GetPosition() << " " << the511RecHits[1].GetPosition() << G4endl;
#endif
    theEventClass += 1;
    G4double linedist;
...
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(infoVerb) ) G4cout << " PETEventClassifierCrystalIDUA: classification " << theEventClass << G4endl;
#endif
  ...
  if( bDumpLM && theEventClass != 0 ) {
    ...
    PETIOMgrCrystalID::GetInstance()->WriteEvent( pvtx->GetPosition(),
					    Evtx,
					    pvtx->GetT0(),
					    the511RecHits[0],
					    the511RecHits[1]);
...
//----------------------------------------------------------------------
void PETEventClassifierCrystalIDUA::Get511ComptonRecHits( const gamosRecHitList& rhitList )
{
  ...
  std::map<G4double,DetRecHitSet> distEnerComptRecHitList; // store in a map usig as key energy distance to electron_mass_c2, so that they can be efficiently delete if there are more than 2
...
#ifndef GAMOS_NO_VERBOSE
	if( PETVerb(debugVerb) ) G4cout <<"PETEventClassifierCrystalIDUA::Get511ComptonRecHits: insert first not used Rechit in set " << seedRecHit->GetPosition() << " E " << seedRecHit->GetEnergy() << G4endl;
#endif
	break;
      }
    }
	...
	if( recHitUsed.find( newRecHit ) == recHitUsed.end() && 
	  ...
	  if( dist < theComptonRecHitsDist ){
	    ...
	    if( PETVerb(debugVerb) ) G4cout 
	      << "PETEventClassifierCrystalIDUA::Get511ComptonRecHits: " 
...
#ifndef GAMOS_NO_VERBOSE
      if( PETVerb(debugVerb) ) G4cout 
	     << "PETEventClassifierCrystalIDUA::Get511ComptonRecHits: rec hit set Energy BAD " 
...
#ifndef GAMOS_NO_VERBOSE
      if( PETVerb(debugVerb) ) G4cout 
	     << "PETEventClassifierCrystalIDUA::Get511ComptonRecHits: rec hit set Energy GOOD " 
...
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout 
	 << "PETEventClassifierCrystalIDUA::Get511ComptonRecHits: building of rec hit sets ended. N sets built = " 
  ...
  //---- If there are more than take one only those that match in coincidence time with another 
  ...
  //--- Order the two sets of rec hits by number of rec hits
  itedss = distEnerComptRecHitList.begin();
  ...
  G4ThreeVector pos;
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierCrystalIDUA::Get511ComptonRecHits: firstRecHitSet initial pos " << firstRecHitSet.GetPosition() << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierCrystalIDUA::Get511ComptonRecHits: firstRecHitSet change pos to " << pos << " biggest E  " << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierCrystalIDUA::Get511ComptonRecHits: firstRecHitSet  pos final " << firstRecHitSet.GetPosition() << G4endl;
#endif
  ...
  if( distEnerComptRecHitList.size() == 2 ) {
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierCrystalIDUA::Get511ComptonRecHits: secondRecHitSet initial pos " << secondRecHitSet.GetPosition() << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
      if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierCrystalIDUA::Get511ComptonRecHits: secondRecHitSet change pos " << pos << G4endl;
#endif
      secondRecHitSet.SetPosition(pos);
      if( PETVerb(debugVerb) ) secondRecHitSet.Print();
    }
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierCrystalIDUA::Get511ComptonRecHits: secondRecHitSet pos final " << secondRecHitSet.GetPosition() << G4endl;
#endif
    //--- Register it as 511 rec hit
...
//----------------------------------------------------------------------
G4bool PETEventClassifierCrystalIDUA::CheckTimeCoincidence( mddrh::iterator itedss1, mddrh::iterator itedss2 )
{
  if( theCoincidenceTime <= 0. ) return true;

#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << " PETEventClassifierUA::CheckTimeCoincidence " << itedss1->second.GetTime() - itedss2->second.GetTime() << " >? " << theCoincidenceTime << " time1= " << itedss1->second.GetTime() << " time2= " << itedss2->second.GetTime() << G4endl;
#endif
...
//----------------------------------------------------------------------
void PETEventClassifierCrystalIDUA::CheckTimeCoincidence()
{
...
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << " PETEventClassifierCrystalIDUA::CheckTimeCoincidence() " << (*ites1).GetTime() - (*ites2).GetTime() << " >? " << theCoincidenceTime << " time1= " << (*ites1).GetTime() << " time2= " << (*ites2).GetTime() << G4endl;
#endif
...
//----------------------------------------------------------------------
G4ThreeVector PETEventClassifierCrystalIDUA::GetEventVertex( const G4Event* evt )
{
...
//----------------------------------------------------------------------
G4double PETEventClassifierCrystalIDUA::GetPETLineDistanceToVtx( const GmLine& line, const G4ThreeVector& eventVtx )
{
  if( the511RecHits.size() != 2 ) {
    G4Exception("PETEventClassifierCrystalIDUA::GetPETLineDistanceToVtx",
		"Error",
		FatalException,
		G4String("Number of 511 recHits !=2 " + GmGenUtils::itoa( the511RecHits.size() ) ).c_str());
  }

  //  if( !foundPositron ) G4Exception("PETEventClassifierCrystalIDUA::GetPETLineDistanceToVtx: no positron found ");

#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << " PETEventClassifierCrystalIDUA::GetPETLineDistanceToVtx recHit 0 pos " << the511RecHits[0].GetPosition()<< " E " << the511RecHits[0].GetEnergy()<< " recHit 1 pos " << the511RecHits[1].GetPosition()<< " E " << the511RecHits[1].GetEnergy() << G4endl;
#endif

  G4double linedist = line.GetDist( eventVtx );
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(infoVerb) ) G4cout << " PETEventClassifierCrystalIDUA::GetPETLineDistanceToVtx " << linedist << " ?< " << thePETLineDistToVtx << G4endl;
#endif
...
//----------------------------------------------------------------------
void PETEventClassifierCrystalIDUA::FillDCAHistos( G4double linedist )
{
...
//----------------------------------------------------------------------
G4bool PETEventClassifierCrystalIDUA::CheckDifferentOriginalGammaAndEvent( DetRecHitSetList& rhitsets )
{
...
//----------------------------------------------------------------------
G4bool PETEventClassifierCrystalIDUA::CheckRecHitDifferentOriginalGammaAndEvent( GmRecHit* rhit, G4int& origGammaAndEvent )
{
...
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << origGammaAndEvent << "PETEventClassifierCrystalIDUA::CheckRecHitDifferentOriginalGammaAndEvent hits in recHit " << hits.size() << G4endl;
#endif
  //  G4double maxdist = 0.;
...
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout 
	   << "PETEventClassifierCrystalIDUA::CheckRecHitDifferentOriginalGammaAndEvent looping hits n tracks " 
	   << htidori.size() << G4endl;
#endif
    for( itesi = htidori.begin(); itesi != htidori.end(); itesi++ ){
      G4int oge = (*itesi)+10*(*iteh)->GetEventID();
#ifndef GAMOS_NO_VERBOSE
      if( PETVerb(debugVerb) ) G4cout << " new track id " << *itesi+10.*(*iteh)->GetEventID() << " ogenew " << oge << " old " << origGammaAndEvent << G4endl;
#endif
...
#ifndef GAMOS_NO_VERBOSE
	if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierCrystalIDUA::CheckRecHitDifferentOriginalGammaAndEvent origGammaAndEvent changed " << origGammaAndEvent << G4endl;
#endif
	return isDifferentG;
      }
    }
...
//----------------------------------------------------------------------
G4bool PETEventClassifierCrystalIDUA::CheckOriginalGammaScattered( DetRecHitSetList& rhitsets )
{
  //----- Loop to all recHits original gammas to check 
  DetRecHitSetList::iterator ites;
  std::set<GmRecHit*>::iterator ite;
...
//----------------------------------------------------------------------
G4bool PETEventClassifierCrystalIDUA::CheckRecHitOriginalGammaScattered( GmRecHit* rhit )
{
  if( rhit == 0 ) return FALSE;
  ...
  std::set<G4int>::iterator itesi;
#ifndef GAMOS_NO_VERBOSE
  if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierCrystalIDUA::CheckRecHitOriginalGammaScattered hits in recHit " << hits.size() << G4endl;
#endif

  for( iteh = hits.begin(); iteh != hits.end(); iteh++ ){
    std::set<G4int> htidori = (*iteh)->GetOriginalTrackIDs();
#ifndef GAMOS_NO_VERBOSE
    if( PETVerb(debugVerb) ) G4cout << "PETEventClassifierCrystalIDUA::CheckRecHitOriginalGammaScattered looping hits n original tracks " << htidori.size() << G4endl;
#endif
    for( itesi = htidori.begin(); itesi != htidori.end(); itesi++ ){
#ifndef GAMOS_NO_VERBOSE
...
//----------------------------------------------------------------------
void PETEventClassifierCrystalIDUA::EndOfRunAction(const G4Run* aRun )
{
  ...
  if( bDumpLM ) PETIOMgrCrystalID::GetInstance()->CloseFileOut();
...
//----------------------------------------------------------------------
void PETEventClassifierCrystalIDUA::PrintPETClassificationTable( int64_t nevt )
{
  theAnaMgr = GmAnalysisMgr::GetInstance("pet") ;
...
//----------------------------------------------------------------------
void PETEventClassifierCrystalIDUA::EndOfEventAction(const G4Event* evt )
{
