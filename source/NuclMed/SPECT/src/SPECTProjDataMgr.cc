...
//----------------------------------------------------------------------
SPECTProjDataMgr* SPECTProjDataMgr::theInstance = 0;

//----------------------------------------------------------------------
SPECTProjDataMgr* SPECTProjDataMgr::GetInstance()
{
  if( !theInstance ) {
    theInstance = new SPECTProjDataMgr;
  }
...
//-----------------------------------------------------------------------
SPECTProjDataMgr::SPECTProjDataMgr()
{
...
" \n"
" SPECT Reconstruction. CIEMAT 2009-11 \n"
" mario.canadas@ciemat.es \n"
 " -------------------------- \n";
  */

  theDistAxial = GmParameterMgr::GetInstance()->GetNumericValue("SPECT:ProjData:DistAxial",100.0);
  theD_FOV = GmParameterMgr::GetInstance()->GetNumericValue("SPECT:ProjData:DiameterTranFOV",300.0);
  theN_planes = G4int(GmParameterMgr::GetInstance()->GetNumericValue("SPECT:ProjData:Nplanes",48));
  theN_bin = G4int(GmParameterMgr::GetInstance()->GetNumericValue("SPECT:ProjData:Nbin",128));
  theN_ang = G4int(GmParameterMgr::GetInstance()->GetNumericValue("SPECT:ProjData:NangViews",128));
  theMRD = G4int(GmParameterMgr::GetInstance()->GetNumericValue("SPECT:ProjData:MaxRingDiff",-1));
  // toDo:  theSpan = G4int(GmParameterMgr::GetInstance()->GetNumericValue("SPECT:ProjData:Span",1));

  theOffsetX = GmParameterMgr::GetInstance()->GetNumericValue("SPECT:ProjData:OffsetX",0.0);
  theMaxCoinci = G4int(GmParameterMgr::GetInstance()->GetNumericValue("SPECT:ProjData:MaxCoinci",-1));

  theOutFormat = G4int(GmParameterMgr::GetInstance()->GetNumericValue("SPECT:ProjData:OutFormat", 1));
  theFileOUTname = GmParameterMgr::GetInstance()->GetStringValue("SPECT:ProjData:Filename","default_sino3D");
...
//-----------------------------------------------------------------------
void SPECTProjDataMgr::AddEvent( const G4ThreeVector& pos1 , const G4ThreeVector& pos2)
{
  G4int z1_i, z2_i; 
  ...
  if ( (pos1.x()==pos2.x()) && (pos1.y()==pos2.y()) ) {
    ...
    if( SPECTVerb(debugVerb) ) {
      G4cout << "SPECTProjDataMgr::AddEvent:WARNING! Event_1 == Event_2  ; x= " << pos2.x() << ", y= " << pos2.y() << ", z= " << pos2.z() << G4endl;
    }
#endif 
    return;
  }
...
#ifndef GAMOS_NO_VERBOSE
    if( SPECTVerb(debugVerb) ) {
      G4cout << "SPECTProjDataMgr::AddEvent:WARNING! Event out of bounds (Axial): x1= " << pos1.x() << ", y1= " << pos1.y() << ", z1= " << pos1.z() << " ; x2= " << pos2.x() << ", y2= " << pos2.y() << ", z2= " << pos2.z() << G4endl;
    }
#endif 
    return;
  }
...
#ifndef GAMOS_NO_VERBOSE
    if( SPECTVerb(debugVerb) ) {
      G4cout << "SPECTProjDataMgr::AddEvent:WARNING! Event out of bounds (Max. Ring Diff.): x1= " << pos1.x() << ", y1= " << pos1.y() << ", z1= " << pos1.z() << " ; x2= " << pos2.x() << ", y2= " << pos2.y() << ", z2= " << pos2.z() << G4endl;
    }
#endif 
    return;
  }
...
#ifndef GAMOS_NO_VERBOSE
    if( SPECTVerb(debugVerb) ) {
      G4cout << "SPECTProjDataMgr::AddEvent:WARNING! Event out of bounds (Transaxial): x1= " << pos1.x() << ", y1= " << pos1.y() << ", z1= " << pos1.z() << " ; x2= " << pos2.x() << ", y2= " << pos2.y() << ", z2= " << pos2.z() << G4endl;
    }
#endif 
    return;
  }
...
#ifndef GAMOS_NO_VERBOSE
  if( SPECTVerb(debugVerb) ) {
    G4cout << "SPECTProjDataMgr::AddEvent: x1= " << pos1.x() << ", y1= " << pos1.y() << ", z1= " << pos1.z() << " ; x2= " << pos2.x() << ", y2= " << pos2.y() << ", z2= " << pos2.z() << G4endl;
    G4cout << "SPECTProjDataMgr::AddEvent: Sinogram pos.: distance(s)= " << dis_i << ", angular view(phi)= " << phi_i << " ; Zpos=" << Zpos <<"; Segment (Ring diff.) = " << ring_diff << G4endl;
  }
#endif
...
//-----------------------------------------------------------------------
SPECTOutput SPECTProjDataMgr::ReadEvent( G4bool& bEof )
{
  if( theFileIn == 0 ){
...
//-----------------------------------------------------------------------
void SPECTProjDataMgr::WriteInterfile()
{
  ...
  if (theOutFormat==0){
    ...
    fprintf (theOUTfile, "!SPECT STUDY (General) :=\n");
    fprintf (theOUTfile, "!SPECT data type := 3D-Sinogram\n");
...
#ifndef GAMOS_NO_VERBOSE
  G4cout << "SPECTProjDataMgr::WriteInerfile: File name: " << theFileOUTname << G4endl;
  G4cout << "SPECTProjDataMgr::WriteInterfile: Planes = " << theN_planes << "; bins = " << theN_bin << "; ang_views = " << theN_ang << G4endl;
  G4cout << "SPECTProjDataMgr::WriteInterfile: Dimensions (mm): Transaxial FOV = " << theD_FOV << "; Axial FOV = " << theDistAxial << " ; Transaxial_pix = " << theD_FOV/(theN_bin-1) <<"; Plane width = " << theDistAxial/(theN_planes-1) << G4endl; // Image Axial Pixel(ssrb) == 0.5*(Plane_Width);
  G4cout << "... " << G4endl;

 G4cout << "SPECTProjDataMgr::WriteInterfile: Total  Coinci: " << theN_CoinciTot << G4endl; 
 G4cout << "SPECTProjDataMgr::WriteInterfile: Sino3D Coinci: " << theN_CoinciVal << G4endl;  
#endif
