...
//----------------------------------------------------------------------
DetCClassifXYPosMin::DetCClassifXYPosMin()
{
  theName = "XYPosMin";
  InitVariables();
}
...
//----------------------------------------------------------------------
void DetCClassifXYPosMin::BookSpecificHistos(G4int histoNumber)
{
  ...
  hnam =  "DetCompton"+sepa+"Classif"+sepa+" RecHit XPos";
...
//----------------------------------------------------------------------
int64_t DetCClassifXYPosMin::GetClassification( DetRecHitSet& prhs, G4int nh )
{
  ...
  for( iterhs = prhs.begin(); iterhs != prhs.end(); iterhs++ ){
    G4double val = (*iterhs)->GetPosition().perp();
