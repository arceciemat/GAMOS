...
//----------------------------------------------------------------------
void DetCAlgoXYZPos::FillHistosInteractions( std::pair<mmits::iterator,mmits::iterator> itemmp, G4int nh )
{
  std::vector<G4double> values;
  mmits::iterator itemm;
  for( itemm = itemmp.first; itemm != itemmp.second; itemm++ ){
    values.push_back( (*itemm).second->GetPositionPost().mag() );
  }
...
//----------------------------------------------------------------------
void DetCAlgoXYZPos::FillHistosRHits( const std::vector<std::pair<GmTrajStep*,GmRecHit*> >& recHitOrdered, G4int nh )
{
  std::vector<G4double> values;
  ...
  for( iterho = recHitOrdered.begin(); iterho != recHitOrdered.end(); iterho++ ){
    ...
    values.push_back( rh->GetPosition().mag() );
  }
