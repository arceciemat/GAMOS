...
//----------------------------------------------------------------------
DetCClassifXPosMin::DetCClassifXPosMin()
{
  theName = "XPosMin";
  InitVariables();
}
...
//----------------------------------------------------------------------
void DetCClassifXPosMin::BookSpecificHistos(G4int histoNumber)
{
  ...
  hnam =  "DetCompton:Classif: RecHit XPos";
...
//----------------------------------------------------------------------
int64_t DetCClassifXPosMin::GetClassification( DetRecHitSet& prhs, G4int nh )
{
  ...
  for( iterhs = prhs.begin(); iterhs != prhs.end(); iterhs++ ){
    G4double val = fabs((*iterhs)->GetPosition().x());
