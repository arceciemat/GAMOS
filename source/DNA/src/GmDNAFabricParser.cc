...
// Molecule struct def
struct GmDNAFabricParser::Molecule
{
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
GmDNAFabricParser::GmDNAFabricParser()
    : fSize(0)
    , fGeoName("")
    , fpWater(nullptr)
...
GmDNAFabricParser::~GmDNAFabricParser() = default;

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

G4LogicalVolume* GmDNAFabricParser::CreateLogicVolume()
{
    ...
    for(int i = 0, ie = fMolecules.size(); i < ie; ++i)
    {
	    ...
	    // G4cout << " 1MOLECULE_PV " << nameWaterPhys << " IN " << pBoxLogic->GetName() << " " <<moleculePosition << " " << copyNum << G4endl; //GDEB
            auto it = fEnumMap.find(nameWaterPhys);
            if(it  != fEnumMap.end())
            {
                fGeometryMap.insert(std::make_pair(pPhysicalVolumeWater,
                                                    it->second));
		// if( it->second == physWorld ) G4cout << " GmDNAFabricParser filling world " << pPhysicalVolumeWater->GetName() << G4endl; //GDEB Never happens!
            }
            else
            {
                ...
                G4Exception("GmDNAFabricParser::CreateLogicVolume()",
                            "GmDNAFabricParser001", FatalException,
...
// Dna volume part
	    ...
	    // G4cout << " 2MOLECULE_PV " << namePhys << " IN " << pMoleculeWaterLogic->GetName() << " " << position << " " << copyNum << G4endl; //GDEB
            ...
            else
            {
                ...
                exceptionDescription <<namePhys
                                     <<" could not be exsited";
                G4Exception("GmDNAFabricParser::CreateLogicVolume()",
                            "GmDNAFabricParser002", FatalException,
                             exceptionDescription);
            }                         
        }
        else
        {
            ...
            pPhysicalVolumeMolecule =  new G4PVPlacement(0,
                                                         ...
                                                         false,
                                                         copyNum);
	    // G4cout << " 3MOLECULE_PV " << namePhys << " IN " << pBoxLogic->GetName() << " " << position << " " << copyNum << G4endl; //GDEB
            ...
            else
            {
                ...
                exceptionDescription <<namePhys
                                     <<" could not be exsited";
                G4Exception("GmDNAFabricParser::CreateLogicVolume()",
                            "GmDNAFabricParser003", FatalException,
                             exceptionDescription);
            }                                
        }
    }
...
void GmDNAFabricParser::ParseFile(const std::string& fileName)
{
    fMolecules.clear();
    fRadiusMap.clear();
    fWaterRadiusMap.clear();
    ...
    if(!file.is_open())
    {
        ...
        exceptionDescription <<fileName
                             <<"could not be opened";
        G4Exception("GmDNAFabricParser::ParseFile()",
                    "GmDNAFabricParser002", FatalException,
                    exceptionDescription);
    }
    std::string line;
    while(std::getline(file, line) )
    {
        ...
        if(firstItem == "_pl")
        {
            ...
            if(itt != fEnumMap.end())
            {
                ...
                if(itt->second != GmDNAFabricVolumeType::phosphate1 && 
                   itt->second != GmDNAFabricVolumeType::phosphate2)
                {
                    if(itt->second == GmDNAFabricVolumeType::deoxyribose1 || 
                       itt->second == GmDNAFabricVolumeType::deoxyribose2)
                    {
                        name = "Deoxyribose";
                    }
                    if(itt->second == GmDNAFabricVolumeType::base_adenine)
                    {
                        name = "Adenine";
                    }
                    if(itt->second == GmDNAFabricVolumeType::base_thymine)
                    {
                        name = "Thymine";
                    }
                    if(itt->second == GmDNAFabricVolumeType::base_cytosine)
                    {
                        name = "Cytosine";
                    }
                    if(itt->second == GmDNAFabricVolumeType::base_guanine)
                    {
                        name = "Guanine";
                    }
                    if(itt->second == GmDNAFabricVolumeType::histone)
                    {
                        name = "Histone";
                    }
            ...
            else
            {
                ...
                exceptionDescription <<name
                                     <<" could not be exsited";
                G4Exception("GmDNAFabricParser::ParseFile()",
                            "GmDNAFabricParser005", FatalException,
                             exceptionDescription);
            }
        }
    } 
...
G4VSolid* GmDNAFabricParser::CreateCutSolid(G4Orb *pSolidOrbRef,
                                        Molecule &molRef,
...
// Backward
    ...
    for(int i=0, ie=molList.size(); i<ie; ++i)
    {
        ...
        else if(distance == 0 && isOurVol)
        {
            ...
            G4Exception("GmDNAFabricParser::CreateCutSolid()",
                        "GmDNAFabricParser004", FatalException,
                        exceptionDescription);
        }
...
void GmDNAFabricParser::EnumParser()
{             
