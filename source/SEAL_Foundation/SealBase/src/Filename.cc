...
#include "SealBase/sysapi/Filename.h"
...
Filename
Filename::relativeTo (const Filename &) const
{
...
Filename::substitute (const Environment &env) const
{
	...
	else
	{
	  ...
	  if (*p == '$')
	    ...
	    std::string name (start2, end - start2);
	    std::string	value;
...
//		const Filename &dir,
Filename::glob (std::vector<Filename> &,
		const Filename &,
