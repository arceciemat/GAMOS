...
#include "SealBase/sysapi/File.h"
...
File::File (const char *name, int flags /*= OpenRead*/, FileAcl perms /*= 0666*/)
{ open (name, flags, perms); }

/** Create a new file object by calling #open() with the given arguments.  */
File::File (const std::string &name, int flags /*= OpenRead*/, FileAcl perms /*= 0666*/)
...
/** Create and open the file @a name in write mode.  If @a exclusive,
    ...
    permissions @a perms. */
void
File::create (const char *name, bool exclusive /*=false*/, FileAcl perms/*=0666*/)
{
    open (name,
...
/** Create and open the file @a name in write mode.  If @a exclusive,
    ...
    permissions @a perms. */
void
File::create (const std::string &name, bool exclusive /*=false*/, FileAcl perms/*=0666*/)
{
    open (name.c_str (),
	  (OpenCreate | OpenWrite | OpenTruncate
	   | (exclusive ? OpenExclusive : 0)),
...
void
File::open (const std::string &name, int flags /*= OpenRead*/, FileAcl perms /*= 0666*/)
{ open (name.c_str (), flags, perms); }
...
/** Open or possibly create the file @a name with options specified in
    ...
    it is closed first.  */
void
File::open (const char *name, int flags /*= OpenRead*/, FileAcl perms /*= 0666*/)
{
    ...
    if (fd () != IOFD_INVALID && (m_flags & InternalAutoClose))
	close ();
...
File::close (void)
{
	...
	throw FileError ("sysclose()", error);

    m_flags &= ~InternalAutoClose;
    this->fd (IOFD_INVALID);
}
...
//////////////////////////////////////////////////////////////////////
logstream &
operator<< (logstream &log, const File &f)
{
    return log << "File @ " << (const void *) &f << " {\n" << indent <<
	"fd = " << f.fd () << undent << "}\n";
}
