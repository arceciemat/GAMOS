#ifndef SEAL_BASE_DEBUG_H
# define SEAL_BASE_DEBUG_H
...
# ifdef SEAL_BASE_FUNCTION
// Define for the function name in <<ASSERT>>.
#  define ASSERT_FUNC SEAL_BASE_FUNCTION
# else
...
#  define ASSERT_ALWAYS(f)	\
		((f)?(void)0: 	\
		 seal::DebugAids::failed (__FILE__,__LINE__,ASSERT_FUNC,#f))
#  define ASSERT(f)		\
		((f)?(void)0:	\
		 seal::DebugAids::failed (__FILE__,__LINE__,ASSERT_FUNC,#f))
#  define VERIFY(f)		\
...
#  define ASSERT_VALID(p)	\
		(seal::ASSERT_VALIDATOR((p),__FILE__,__LINE__,ASSERT_FUNC,#p))

# else // NDEBUG
#  define ASSERT_ALWAYS(f)	\
		((f)?(void)0:	\
		 seal::DebugAids::failed (__FILE__,__LINE__,ASSERT_FUNC,0))
