...
int seal_test::TimeInfo01(int, char **argv)
{
    ...
    seal_test::out << "sampling overhead (1 and " << NSAMPLES << " samples)\n"
	      << "  virtual 0   : " << (v1-v0) << "\n"
	      << "  virtual 1   : " << (v2-v1) << "\n"
	      << "  virtual end : " << (vend-v2) << "\n"
    ...
    CPPUNIT_ASSERT( (r1 - r0) > 0); 
    CPPUNIT_ASSERT( (r2 - r1) > 0); 
    CPPUNIT_ASSERT( (rend-r2) / NSAMPLES /  TimeInfo::ghz () > 0
		    && (rend-r2) / NSAMPLES /  TimeInfo::ghz () < 1000 ); 
    CPPUNIT_ASSERT( (vend-v2) / NSAMPLES /  TimeInfo::ghz () > 10
    ...
    seal_test::out << "measuring time in a loop ...." << std::endl; 
    ...
    seal_test::out << " HR Real Time (s) " <<  (realNsec1-realNsec0)*1e-9
		   << " Virtual Time (s) " <<  (virtNsec1-virtNsec0)*1e-9 
		   << std::endl;

    // check that user + sys = cpu 
    CPPUNIT_ASSERT_DOUBLES_EQUAL( cpuTime, userTime + sysTime, TimeInfo::resolution()/2); 
    CPPUNIT_ASSERT_DOUBLES_EQUAL(realTime, cpuTime+idleTime,TimeInfo::resolution()); 
    CPPUNIT_ASSERT_DOUBLES_EQUAL(realTime, (realNsec1-realNsec0)*1e-9, TimeInfo::resolution() ); 
    CPPUNIT_ASSERT_DOUBLES_EQUAL(cpuTime, (virtNsec1-virtNsec0)*1e-9, TimeInfo::resolution() ); 
    CPPUNIT_ASSERT( (realNsec1-realNsec0) > (virtNsec1-virtNsec0) ); 
