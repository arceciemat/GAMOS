...
import sys
def usage():
    print 'Usage: runtests.py [-h] [-V] [--list] [--colors] [-g pdfname] -H histosFile outfile [-T textFile text_qafile_1 text_qafile_2 ...]'
...
def run( argv = sys.argv ):
    ...
    if len(argv)>=1:
        import getopt
        try:
            opts,args = getopt.getopt( argv ,"hvVT:H:g:",["list","colors"])
...
#            print("OP AR %s %s"%(op,ar))
        ...
        if len(args)>=1:
            try:
                ...
                errcode = runQA( inputText, inputHistos, args[0], qafiles , doText = (inputText!="") , doHistos = (inputHistos!="") ) 
            except Exception,e:
